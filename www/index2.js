...
function aedinfo(aed){
    ...
    /*
    if (aed.location == undefined){
        aed.location = ""
    }
    if (aed.name == undefined){
        aed.name = ""
    }
    */
...
async function main(){
        let cords = await getCoords() //{"lat": 48.84474935885055,"lon":2.346712105938869} 
        console.log(cords)
        ...
        var map = L.map('map').setView([cords.lat, cords.lon], 17);
            L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
                ...
                for (let i =0; i < AEDs.length; i++){
                    /*
                    L.DomUtil.create("img");
                    L.src ="aed.svg";
                    L.height = "30px";
                    L.onclick = "console.log('test')";
                    return L
                    */
